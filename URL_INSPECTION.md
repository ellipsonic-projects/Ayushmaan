...
> **Note on path segments:** `slug`, `tenant`, `id`, and `consultant_id` are currently *literal* folder names (static routes), not dynamic `[param]` segments — visit them exactly as written. The only dynamic segment is `[id]` under appointments, marked below; substitute any value (e.g. `1`).
...
| `/superadmin/tenants/id/appointments/[id]` *(dynamic — e.g. `/superadmin/tenants/id/appointments/1`)* | `(platform)/superadmin/tenants/id/appointments/[id]/page.tsx` |
...
| `/slug/tenant/admin/appointments/[id]` *(dynamic — e.g. `/slug/tenant/admin/appointments/1`)* | `(tenant)/slug/tenant/(admin)/admin/appointments/[id]/page.tsx` |
