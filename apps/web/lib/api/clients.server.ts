...
export interface OwnClientCase {
  id: string;
  status: string;
  category: string;
  matterKey: string | null;
...
// A consented guardian_links profile (schema §3.7) this client can book
// appointments on behalf of — same Cases shape as the client's own, minus
// further nested dependents (guardian_links doesn't chain).
export interface OwnClientDependent {
  id: string;
  fullName: string;
  dob: string | null;
  user: { email: string; phone: string | null };
