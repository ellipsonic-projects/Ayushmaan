...
// One discrete bookable instant, stepped from a template's startTime..endTime
// by slotDurationMins — the exact same shape/values the client booking flow
// picks a time from (see generateDiscreteAvailability in availability.service.ts).
export interface ClientVisibleAvailabilitySlot {
  start: string;
  end: string;
  durationMins: number;
  dateKey: string;
...
// Same endpoint as getTenantConsultantAvailability, but also surfaces
// `clientVisibleSlots` — the discrete instants a CLIENT sees on book-page —
// for the consultant/admin "Availability" page's preview so it can render
// exactly what clients can book instead of raw template windows.
export async function getTenantConsultantAvailabilityDetail(consultantId: string): Promise<{
  slots: ConsultantAvailabilitySlot[];
  clientVisibleSlots: ClientVisibleAvailabilitySlot[];
}> {
  const empty = { slots: [], clientVisibleSlots: [] };
  const supabase = await createSupabaseServerClient();
  const {
    data: { session },
  } = await supabase.auth.getSession();
  ...
  const { data, clientVisibleSlots } = await res.json();
  return {
    slots: data as ConsultantAvailabilitySlot[],
    clientVisibleSlots: clientVisibleSlots as ClientVisibleAvailabilitySlot[],
  };
}
