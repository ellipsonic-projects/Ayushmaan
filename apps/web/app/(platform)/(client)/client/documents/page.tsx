  ...
  return (
    <div className="flex items-start gap-2.5 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-amber-700 dark:text-amber-400">
      ...
      <p className="text-xs leading-relaxed">
        Documents you upload will automatically be viewable by Consultants. You can fine-tune edit
        and download access for each consultant after uploading.
      </p>
    </div>
  );
}
    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Add a file to share with your care team</DialogDescription>
        </DialogHeader>
              ...
              <div key={c.id} className="flex flex-col gap-3 py-3 first:pt-0 last:pb-0">
                ...
                <div className="grid grid-cols-3 gap-2 rounded-lg border border-border p-2.5">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Switch id={`${doc.id}-${c.id}-view`} defaultChecked={perm.view} />
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    <Label htmlFor={`${doc.id}-${c.id}-edit`} className="text-xs">
                      Edit
                    </Label>
                    <Switch id={`${doc.id}-${c.id}-edit`} defaultChecked={perm.edit} />
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    <Label htmlFor={`${doc.id}-${c.id}-download`} className="text-xs">
                      Download
                    </Label>
                    <Switch id={`${doc.id}-${c.id}-download`} defaultChecked={perm.download} />
                  </div>
                </div>
              </div>
            );
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        ...
        <CardContent className="flex flex-col divide-y divide-border">
          {documents.map((doc) => {
            const sharedWith = Object.values(doc.permissions).filter((p) => p.view).length;
            return (
              <div
                key={doc.id}
