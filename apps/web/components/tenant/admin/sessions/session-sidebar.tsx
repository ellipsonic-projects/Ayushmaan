  ...
  return (
    <div className="border-t border-border pt-3">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between text-sm font-semibold tracking-[-0.01em] text-foreground"
      >
        {title}
        {open ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
      </button>
      {open && <p className="mt-2 text-xs text-muted-foreground">No {title.toLowerCase()} yet.</p>}
    </div>
  );
}
          ...
          {members.map((member) => (
            ...
            >
              <input
                ...
                className={cn("h-4 w-4 rounded border-border text-primary accent-primary")}
              />
              {member.colorClass && (
                <span
