...
import { submitGrievanceEscalation } from "@/lib/api/grievances.client";
...
const SEVERITIES = [
  ...
  },
  {
    ...
    dot: "bg-orange-500",
    active:
      "border-orange-400 bg-orange-50 text-orange-700 dark:border-orange-800 dark:bg-orange-950 dark:text-orange-400",
  },
  {
    value: "CRITICAL",
    label: "Critical",
...
export function RaiseGrievanceForm() {
  ...
  const isValid = subjectType.length > 0 && category.length > 0 && description.trim().length > 0;
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setError(null);
    try {
      // Attachments aren't persisted yet — no signed-upload flow is wired
      // up for this form in this pass, so only the description/category/
      // severity fields are submitted.
      await submitGrievanceEscalation({
        subjectType: subjectType as "BILLING" | "PLATFORM" | "OTHER",
        category: category as "BILLING_DISPUTE" | "DATA_PRIVACY" | "OTHER",
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Card
        className={cn(
          "border-l-4",
          severityBorderClass[severity as keyof typeof severityBorderClass] ?? "border-l-blue-500"
        )}
      >
        <CardHeader>
          ...
          <CardDescription>
            Ayushman&apos;s platform team is notified immediately on submit. You can track its
            status below.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-5">
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                {attachments.map((name) => (
                  <Badge key={name} variant="secondary" className="gap-1 py-1">
                    {name}
                    <button type="button" onClick={() => removeAttachment(name)} className="ml-0.5">
                      <X className="h-3 w-3" />
                    </button>
                  </Badge>
                ))}
          ...
          {submitted && (
            <p className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-400">
              Escalation submitted. The platform team has been notified and you can track its status
              below.
            </p>
          )}
