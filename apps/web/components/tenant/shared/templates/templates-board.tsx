...
import { Download, FileText, Folder, MoreVertical, Plus, Star } from "lucide-react";
...
const organizationTemplates: TemplateCard[] = [
  { id: "org-consent", name: "Standard Consent Form", type: "Forms", editedOn: "Jul 2, 2026", author: "Org", active: true },
  { id: "org-progress-note", name: "Progress Note", type: "Notes", editedOn: "Jun 28, 2026", author: "Org", active: true },
  { id: "org-discharge", name: "Discharge Summary", type: "Plans & reports", editedOn: "Jun 20, 2026", author: "Org", active: true },
];

const communityTemplates: CommunityTemplate[] = [
  { id: "com-biopsychosocial", name: "Biopsychosocial Assessment", type: "Assessments", author: "Dr. R. Patel", uses: 4210, rating: 4.8 },
  { id: "com-cbt-worksheet", name: "CBT Thought Record", type: "Worksheets & handouts", author: "M. Chen, LCSW", uses: 3190, rating: 4.7 },
  { id: "com-treatment-plan", name: "Treatment Plan Template", type: "Plans & reports", author: "A. Whitfield", uses: 2870, rating: 4.6 },
  { id: "com-telehealth-consent", name: "Telehealth Consent", type: "Forms", author: "S. Okafor", uses: 1980, rating: 4.5 },
];
...
export function TemplatesBoard() {
  ...
  return (
    <Tabs defaultValue="my" className="flex flex-col gap-4">
      ...
      <TabsContent value="my" className="flex flex-col gap-6">
        <div>
      ...
      <TabsContent value="community">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {communityTemplates.map((template) => (
            <Card key={template.id} size="sm" className="transition-shadow hover:shadow-md">
              <CardContent className="flex flex-col gap-2">
                <div className="flex items-start justify-between gap-2">
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-muted">
                    <FileText className="h-4 w-4 text-muted-foreground" />
                ...
                <div>
                  <p className="truncate text-sm font-medium text-foreground">{template.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {template.type} &middot; by {template.author}
                  </p>
                </div>
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  <Button size="sm" variant="outline" onClick={() => openEditor(template.id)}>
                    Use template
                  </Button>
                </div>
              </CardContent>
