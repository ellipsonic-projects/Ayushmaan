...
import { ReferColleagueDialog } from "@/components/tenant/consultant/case-detail/refer-colleague-dialog";
import { CaseTagsManager } from "@/components/tenant/consultant/case-detail/case-tags-manager";
import { CaseStatusToggle } from "@/components/tenant/consultant/case-detail/case-status-toggle";
    ...
    <aside className="sticky top-5 flex w-full shrink-0 flex-col gap-4 lg:w-72">
      <Card>
        ...
        <CardContent className="flex flex-col gap-3">
          ...
          {caseDetail.client.user.phone ? (
            <div className="flex items-start gap-2.5">
              <Phone className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
              <div className="flex min-w-0 flex-1 items-center justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-xs text-muted-foreground">Phone</p>
                  <p className="truncate text-sm text-foreground">{caseDetail.client.user.phone}</p>
                </div>
                <div className="flex shrink-0 items-center gap-1">
                  <Button variant="ghost" size="icon-sm" aria-label="Call client" asChild>
                    <a href={`tel:${caseDetail.client.user.phone}`}>
                      <Phone className="h-4 w-4" />
                    </a>
                  </Button>
                  <Button variant="ghost" size="icon-sm" aria-label="WhatsApp client" asChild>
                    <a
                      href={`https://wa.me/${caseDetail.client.user.phone.replace(/\D/g, "")}`}
                      target="_blank"
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-3">
          ...
          />
          <CaseTagsManager caseId={caseDetail.id} tags={caseDetail.tags} />
          {ownConsultantId === caseDetail.consultant.id && (
            <CaseStatusToggle caseId={caseDetail.id} status={caseDetail.status} />
          )}
        </CardContent>
      </Card>
