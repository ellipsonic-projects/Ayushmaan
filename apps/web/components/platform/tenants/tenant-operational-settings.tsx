...
export function TenantOperationalSettings({
  ...
  const [cutoff, setCutoff] = useState<number[]>([bookingCutoffHours]);
  const [autoApprove, setAutoApprove] = useState(autoApproveBookings);
    ...
    <Card size="sm">
      <CardContent className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5">
            <Label className="text-xs font-medium text-muted-foreground">Payout Cycle</Label>
            <Select defaultValue={payoutCycle}>
              <SelectTrigger className="h-8 w-full">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-medium text-foreground">{cutoff[0]} Hours</span>
          </div>
          <Slider
            min={1}
            max={72}
            step={1}
            value={cutoff}
            onValueChange={(value) => setCutoff(Array.isArray(value) ? [...value] : [value])}
          />
        </div>
