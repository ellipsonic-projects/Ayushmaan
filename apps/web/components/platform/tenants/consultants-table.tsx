...
function EditConsultantDialog({
  ...
  const [languagesSpoken, setLanguagesSpoken] = useState<string[]>(consultant.languagesSpoken);
            ...
            <div className="flex flex-wrap items-center gap-2">
              {languagesSpoken.map((language) => (
                <Badge key={language} variant="secondary" className="gap-1 py-1">
                  ...
                  <button
                    type="button"
                    onClick={() => setLanguagesSpoken((prev) => prev.filter((l) => l !== language))}
                    className="ml-0.5"
                  >
              ...
              <Select<string>
                onValueChange={(v) =>
                  v && setLanguagesSpoken((prev) => (prev.includes(v) ? prev : [...prev, v]))
                }
              >
                ...
                <SelectContent>
                  {LANGUAGE_OPTIONS.filter((l) => !languagesSpoken.includes(l.value)).map(
                    (lang) => (
