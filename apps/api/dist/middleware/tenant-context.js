...
const tenantContextMiddleware = async (req, res, next) => {
    ...
    const consultantId = req.user.role === "CONSULTANT"
        ? await (0, rls_context_1.withTenantContext)({ tenantId: null, isSuperAdmin: true, userId: SYSTEM_LOOKUP_USER_ID }, (tx) => (0, callerProfile_1.getOwnConsultantProfileId)(tx, req.user.id))
        : null;
    req.tenant = {
        id: tenant.id,
        slug: tenant.slug,
