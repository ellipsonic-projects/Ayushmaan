...
const workflow_node_handlers_1 = require("./workflow-node-handlers");
...
async function executeNextStep(runId) {
    return (0, rls_context_1.withTenantContext)({ tenantId: null, isSuperAdmin: true, userId: SYSTEM_USER_ID }, async (tx) => {
        ...
        if (!run || run.deletedAt || run.status !== "RUNNING")
            return "halt";
        const graph = run.workflow.graph;
        ...
        const result = resumingPastWait
            ? {}
            : await workflow_node_handlers_1.nodeHandlers[node.type](tx, run.tenantId, run.id, node, context);
        if (result.resumeAt) {
            await tx.workflowRun.update({
                where: { id: run.id },
...
async function markRunFailed(runId, reason) {
    await (0, rls_context_1.withTenantContext)({ tenantId: null, isSuperAdmin: true, userId: SYSTEM_USER_ID }, (tx) => tx.workflowRun.update({
        where: { id: runId },
