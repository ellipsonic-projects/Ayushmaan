...
exports.generateDiscreteAvailability = generateDiscreteAvailability;
exports.applyTenantAvailabilityDefaults = applyTenantAvailabilityDefaults;
...
function generateDiscreteAvailability(templates, appointments, from, to, cutoffMs, now) {
    const results = [];
    const fromDay = Date.UTC(from.getUTCFullYear(), from.getUTCMonth(), from.getUTCDate());
    const toDay = Date.UTC(to.getUTCFullYear(), to.getUTCMonth(), to.getUTCDate());
    for (let dayMs = fromDay; dayMs <= toDay; dayMs += 86400000) {
        const day = new Date(dayMs);
        const dateKey = day.toISOString().slice(0, 10);
        const weekday = day.getUTCDay();
        const specificMatches = templates.filter((t) => t.specificDate && t.specificDate.toISOString().slice(0, 10) === dateKey);
        const dayTemplates = specificMatches.length > 0
            ? specificMatches
            : templates.filter((t) => !t.specificDate && t.dayOfWeek === weekday);
        for (const template of dayTemplates) {
            const stepMs = template.slotDurationMins * 60000;
            const startOfWindowMs = dayMs +
                template.startTime.getUTCHours() * 3600000 +
                template.startTime.getUTCMinutes() * 60000;
            const endOfWindowMs = dayMs +
                template.endTime.getUTCHours() * 3600000 +
                template.endTime.getUTCMinutes() * 60000;
            ...
            while (cursorMs + stepMs <= endOfWindowMs) {
                ...
                const conflicts = appointments.some((a) => a.scheduledStart < end && a.scheduledEnd > start);
...
async function applyTenantAvailabilityDefaults(tx, tenantId, consultantId) {
    ...
    const windowKey = (startTime, endTime) => `${startTime.getTime()}:${endTime.getTime()}`;
    const defaultKey = (d) => `${d.dayOfWeek}:${windowKey(d.startTime, d.endTime)}:${d.slotDurationMins}`;
    const staleIds = [];
    const toCreate = [];
    const bookedUpdates = [];
    ...
    for (const existing of existingSlots) {
        ...
        const matchingWindow = dayDefaults.find((d) => windowKey(d.startTime, d.endTime) === windowKey(existing.startTime, existing.endTime));
        if (!matchingWindow) {
    ...
    for (const { id, slotDurationMins } of bookedUpdates) {
        ops.push(tx.availabilitySlot.update({
            where: { id },
    ...
    return Promise.all(toCreate.map((d) => tx.availabilitySlot.create({
        data: {
            tenantId,
            consultantId,
