...
exports.platformGrievancesRouter.get("/", async (req, res) => {
    ...
    const grievances = await (0, rls_context_1.withTenantContext)({ tenantId: null, isSuperAdmin: true, userId: req.user.id }, async (tx) => {
        ...
        const tenantIds = [
            ...new Set(found.map((g) => g.tenantId).filter((id) => id !== null)),
        ];
        ...
        const clientIds = [
            ...new Set(found.map((g) => g.clientId).filter((id) => id !== null)),
        ];
        ...
        const consultantIds = [
            ...new Set(found.map((g) => g.subjectConsultantId).filter((id) => !!id)),
        ];
