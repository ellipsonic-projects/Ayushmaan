...
exports.clientsRouter.post("/", (0, require_role_1.requireRole)("TENANT_ADMIN", "SUPER_ADMIN"), async (req, res) => {
    ...
    const profile = await (0, rls_context_1.withTenantContext)({ tenantId: null, isSuperAdmin: true, userId: req.user.id }, async (tx) => {
        ...
        select: {
            ...
            consultant: {
                select: {
                    ...
                    ratingCount: true,
                    _count: {
                        select: { cases: { where: { status: "ACTIVE" } } },
                    },
                },
            },
            appointments: {
...
exports.platformClientsRouter.get("/tenants", (0, require_role_1.requireRole)("CLIENT"), async (req, res) => {
    ...
    const tenants = await (0, rls_context_1.withTenantContext)({ tenantId: null, isSuperAdmin: false, userId: req.user.id }, async (tx) => {
        let categoryTenantIds;
        if (query.category) {
...
const searchConsultantsQuerySchema = zod_1.z.object({
...
exports.platformClientsRouter.get("/consultants", (0, require_role_1.requireRole)("CLIENT"), async (req, res) => {
    const query = searchConsultantsQuerySchema.parse(req.query);
    const consultants = await (0, rls_context_1.withTenantContext)({ tenantId: null, isSuperAdmin: false, userId: req.user.id }, async (tx) => {
