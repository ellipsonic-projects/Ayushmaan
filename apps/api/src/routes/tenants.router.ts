...
import { dispatchTenantApprovalDecision } from "../services/notification.service";
...
const listTenantsQuerySchema = z.object({
  status: z.enum(["PENDING_APPROVAL", "ACTIVE", "SUSPENDED", "REJECTED", "ARCHIVED"]).optional(),
...
async function findPendingTenant(tx: Prisma.TransactionClient, tenantId: string) {
  const tenant = await tx.tenant.findUnique({ where: { id: tenantId } });
  if (!tenant) throw new AppError(404, "Tenant not found", "TENANT_NOT_FOUND");
  if (tenant.status !== "PENDING_APPROVAL") {
    throw new AppError(409, "Tenant signup has already been reviewed", "TENANT_ALREADY_REVIEWED");
  }
  return tenant;
}
...
platformTenantsRouter.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    withTenantContext({ tenantId: null, isSuperAdmin: true, userId: req.user!.id }, (tx) =>
      dispatchTenantApprovalDecision(tx, { tenantId: tenant.id, approved: true })
    ).catch((err) => {
...
const rejectTenantSchema = z.object({ reason: z.string().min(1).max(500) }).strict();
...
platformTenantsRouter.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    withTenantContext({ tenantId: null, isSuperAdmin: true, userId: req.user!.id }, (tx) =>
      dispatchTenantApprovalDecision(tx, {
        tenantId: tenant.id,
        approved: false,
        rejectionReason: body.reason,
      })
