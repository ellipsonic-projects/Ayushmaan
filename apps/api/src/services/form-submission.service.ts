...
export async function createAndSendFormSubmission(
  ...
  if (!template || template.deletedAt) {
    console.warn(
      `[form-submission] template ${formTemplateId} not found — skipping ${channel} send`
    );
    return;
  }
  ...
  await sendEmail(
    client.email,
    `Please fill out: ${template.name}`,
    wrapEmailHtml(textToHtml(message))
  );
}
