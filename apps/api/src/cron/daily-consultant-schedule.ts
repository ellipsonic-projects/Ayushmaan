...
import { dispatch, alreadyNotified } from "../services/notification.service";
...
export async function sweepDailyConsultantSchedule(): Promise<void> {
  const now = new Date();
    ...
    async (tx) => {
      ...
      const byConsultant = new Map<
        string,
        {
          ...
          timezone: string;
          appointments: (typeof appointments)[number][];
          firstStart: Date;
        }
      >();
      ...
      for (const [, info] of byConsultant) {
        ...
        if (msUntilFirst < 0 || msUntilFirst > WINDOW_MS) continue;
...
export function startDailyConsultantScheduleCron(): void {
  cron.schedule("* * * * *", () => {
