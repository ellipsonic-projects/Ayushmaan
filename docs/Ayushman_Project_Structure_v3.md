...
│   └── storage-policies/
│       └── tenant-case-prefix.sql                    # {tenantId}/{caseId}/... path scoping for Supabase Storage
```
...
`apps/api/src/routes/payments.router.ts` and the ERD in `schema_ayushman_v3.md` §1 both reference a `payments` table (e.g., `appointments ──► payments`), but no such table is actually defined in that schema's §3 table list — this looks like a gap from the v2→v3 schema rewrite, not something introduced here. I've kept the router in this structure since PRD §5 Phase 8 clearly expects a payments feature, but until the schema has an actual `payments` table, `payments.router.ts` has nothing to persist against. Want me to add that table to the schema now, or is there a reason it's intentionally out of this draft?
