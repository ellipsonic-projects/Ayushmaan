# Ayushman — Step-by-Step Development Sprints (v3, Multi-Tenant Monorepo)

This document outlines the sprint plan for building **Ayushman** on the v3 architecture. It is derived directly from `PRD_v3_nextjs_express.md` and `schema_ayushman_v3.md` — every sprint task is grounded in a specific PRD section or schema table, not carried over from any prior sprint plan. Sprints are sequenced the way this system actually has to be built: **tenancy and identity before anything else** (nothing else works without a tenant and a verified role), then the booking loop that is the core product, then session logging (the core value proposition), then the features that depend on a live client base (growth/analytics, payments, AI, reviews/oversight), then hardening and launch.

---

## 🛠️ Technology Stack & Foundations

_(PRD §7 — Tech Stack & Architecture)_

- **Monorepo**: Turborepo — `apps/web` (Next.js), `apps/api` (Express), `packages/db` (Prisma), `packages/types`, `packages/ui`, `packages/config`
- **Frontend**: Next.js 16 (App Router), React 19, TypeScript, Tailwind v4, SWR, Zod, Radix icons
- **Backend**: Express, Prisma, Supabase PostgreSQL with Row-Level Security, Node-cron
- **Auth**: Supabase Auth (email/phone OTP + password), tenant/role claims stamped via a Postgres Auth Hook, verified independently by `apps/web` (`@supabase/ssr`) and `apps/api` (`@supabase/supabase-js`) — no custom JWT/bcrypt layer
- **Storage**: Supabase Storage, tenant-prefixed bucket paths
- **Vector Store / RAG**: Pinecone, namespaced per tenant, hard-scoped by `tenantId` + `caseId` at the retrieval-service layer
- **Transcription**: Whisper via Hugging Face — dispatched from `apps/api`, resolved asynchronously via a webhook callback (no separate queue infra specified in the PRD; `transcription_status` on `interactions` tracks job state)
- **Payments**: Razorpay
...
1. Initialize the Turborepo at the project root (`turbo.json`, root `package.json` with `workspaces: ["apps/*", "packages/*"]`).
2. Scaffold `apps/web` (Next.js 16, App Router, TypeScript, Tailwind v4) and `apps/api` (Express, TypeScript) as separate workspace packages — confirm `apps/web` has no Prisma/DB dependency from day one (PRD §7.2).
3. Scaffold `packages/db`, `packages/types`, `packages/config` as empty, importable workspace packages.
4. Configure shared `tsconfig` base + path aliases via `packages/config`.
5. Set up ESLint/Prettier across the monorepo; Husky + `lint-staged` for pre-commit.
6. GitHub Actions CI (`.github/workflows/ci.yml`) running lint/typecheck/build across all `apps/*` and `packages/*` via `turbo run`.

### Sprint 0.2: Prisma Schema & Migrations (`packages/db`)

1. Port every table from `schema_ayushman_v3.md` §3 into `packages/db/prisma/schema.prisma` — all 25 tables, including enums (`user_role`, `consultant_category`, `appointment_status`, etc.).
2. Sequence migrations per the dependency order in the schema's own §6 Migration Strategy:
   - `tenants` + `users` first, **without** the `provisioned_by` FK (avoids the circular dependency), then a follow-up migration adding `fk_tenants_provisioned_by`.
   - Client/consultant profile tables (`client_profiles`, `client_category_profiles`, `guardian_links`, `consultant_profiles`, `availability_slots`, `out_of_office_periods`).
   - Booking core (`cases`, `appointment_series`, `appointments`).
   - Session logging (`interactions`, `commitment_templates`, `commitments`, `tasks`, `task_reminders`, `documents`).
   - AI/RAG (`chat_messages`, `ai_summaries`, `rag_citations`).
   - `reviews`, `grievances`.
   - Growth/analytics (`consultant_analytics_snapshot`, `referrals`, `consultant_referrals`).
   - `notifications`, `notification_preferences`, `audit_logs`, `push_subscriptions`.
   - **`consultant_verification_documents` last**, in its own migration — nothing else in the schema depends on this table existing (schema §3.25), so it should not block any earlier migration.
3. Export a single Prisma client singleton from `packages/db/src/client.ts` — the only import path either `apps/api` route needs.
4. Write `withTenantContext()` in `packages/db/src/rls-context.ts`: wraps a Prisma transaction with the `SET LOCAL app.tenant_id` / `SET LOCAL app.is_super_admin` calls RLS depends on (schema §4.1) — build this now so every later sprint's routes can just call it.
5. Write `packages/db/prisma/seed.ts`: one demo tenant, one `TENANT_ADMIN`, one `CONSULTANT` per category enum value, so later sprints have something to develop against.

### Sprint 0.3: Supabase Provisioning & RLS Skeleton

1. Provision a development Supabase project (Postgres, Auth, Storage).
2. Implement the Auth Hook (`supabase/auth-hooks/stamp-tenant-claim.sql`) that stamps `tenant_id`/`is_super_admin` onto the JWT at sign-in (PRD §7.3) — this has to exist before Sprint 1.2's login flow can be tested end-to-end.
3. Write the RLS policy set from schema §4 as raw SQL under `supabase/policies/`:
   - The generic tenant-isolation policy (§4.1), applied to every tenant-scoped table.
   - The `tenant_admin_view_case()` `SECURITY DEFINER` escalation function (§4.2).
   - The `grievances`-specific exception (§4.3) — deliberately no `TENANT_ADMIN` policy branch.
4. Configure Supabase Storage buckets matching the tenant-prefixed path convention (`{tenantId}/{caseId}/...`, `{tenantId}/{consultantId}/...`) and write the storage policy under `supabase/storage-policies/tenant-case-prefix.sql`.
5. Populate `.env.example` at the root: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ANON_KEY`, `DATABASE_URL`, plus placeholders for Pinecone/Razorpay/Twilio/Resend/Hugging Face — leave real secrets to be added locally.
6. Integration test: seed a tenant + two users of different roles, confirm a session-scoped query set to one tenant's `app.tenant_id` cannot read the other tenant's row.
...
1. Build `apps/api`'s `tenants.router.ts`: create tenant (provisions `tenants` + `tenant_settings` + `tenant_billing` + the default `TENANT_ADMIN` `users` row in one transaction), list/search tenants, suspend/reinstate/archive.
2. Build `apps/web/app/(platform)/tenants/page.tsx` and `[tenantId]/page.tsx` against those endpoints.
3. Build `apps/web/app/(platform)/dashboard/page.tsx` — cross-tenant KPI shell (active tenants, MRR placeholder from `tenant_billing`) — real numbers can be stubbed until Phase 6/7 exist.
4. Enforce `SUSPENDED`/unknown-tenant handling in `apps/web/middleware.ts` (UI-level block) — this is the first thing that exercises `apps/web`'s tenant-resolution layer described in PRD §1.2.

### Sprint 1.2: Supabase Auth Integration

1. Wire `apps/web`'s `(auth)/login`, `reset-password`, and `register` pages to Supabase Auth directly via `@supabase/ssr` (PRD §7.3 — not through `apps/api`). All three live under `(auth)/`, not under `(tenant)/[slug]`.
2. Implement role-aware post-login redirect: `SUPER_ADMIN` → platform console, others → their tenant subdomain; detect and reject a token whose `tenant_id` claim doesn't match the subdomain being accessed.
3. Implement `apps/api`'s `users.router.ts`: on first Supabase sign-in for a given `supabase_auth_user_id`, resolve/create the matching `public.users` row (role + `tenant_id` sourced from the JWT claims, never from client input).
4. Tenant-branded password-reset/OTP **email templates only**, configured through Supabase Auth's template settings per tenant where supported — this is separate from and does not imply a tenant-branded login/reset-password _page_.
5. **Acceptance check for this sprint:** load `/login` on two different tenant subdomains side by side — pixel-identical, no `theme_config` fetch in the network tab for either. Only the redirect target after auth (and, later, the `(public)` landing page) should differ per tenant.

### Sprint 1.3: Tenant-Scoping Middleware (the real enforcement boundary)

1. Build `apps/api/src/middleware/tenant-context.ts`: verify the Supabase JWT (`auth.getUser()`), extract `tenant_id`/`is_super_admin`/role, open a `withTenantContext()` transaction for the request (PRD §7.3, schema §4.1).
2. Build `require-role.ts` middleware mirroring the PRD §1.4 permission matrix.
3. Add `apps/api`'s global `error-handler.ts`, including a distinct error path for "JWT valid but tenant mismatch."
4. Integration test suite (`apps/api/tests/integration/rls-policies.test.ts`): confirm a `CONSULTANT` token can't read another tenant's `cases`, and that a `SUPER_ADMIN` token can — while writing an `audit_logs` row every time it does (schema §4, PRD §1.2's "unrestricted ≠ invisible").
...
1. Build `cases.router.ts`: case status transitions (`ACTIVE`/`CLOSED`), tag CRUD (`cases.tags[]`, consultant-private CRM segmentation, schema §3.11).
2. Build `interactions.router.ts`: create a `SESSION_NOTE` tied to an `appointment_id`, or an `AD_HOC_NOTE`/`CALL_LOG`/`MESSAGE_LOG` with no appointment link, for logging a thought between sessions.
3. Build the "quick-capture" floating widget in `apps/web` (available from anywhere in the consultant dashboard, not just inside a session).
4. Implement the `is_client_visible` toggle at write time — defaults to `false` (private) for every interaction type.
5. Implement soft-delete (`deleted_at`) with a 30-day recovery window, per schema §5 retention policy — no hard delete from the API directly.
...
1. Build `documents.router.ts`: upload issues a Supabase Storage signed URL scoped to the `{tenantId}/{caseId}/...` prefix (never a raw bucket credential, per schema §1.2/§3.16).
...
### Sprint 4.4: Commitment Templates, Commitments & Tasks

1. Build `commitment-templates.router.ts` (per-consultant reusable library, schema §3.14) and inline "save as template" from the session-logging editor.
2. Build `commitments.router.ts` and `tasks.router.ts`/`task_reminders.router.ts`, with `assigned_to` distinguishing Consultant to-dos from Client to-dos (schema §3.15).
3. `apps/api/src/cron/`: sweep `tasks` past `due_at` and flag `OVERDUE`; sweep `task_reminders` where `sent_at IS NULL` and dispatch via the notification service (built in Phase 5, so this job can be stubbed to a log line until then).

### Sprint 4.5: Chronological Case Timeline

1. Build `apps/web/.../clients/[caseId]/page.tsx`: unify `appointments`, `interactions`, `commitments`, `tasks`, `documents` into one filterable, searchable, paginated feed, respecting `is_client_visible` when rendered for a Client.
