...
1. Wire `apps/web`'s `(auth)/login`, `reset-password`, and `(tenant)/[slug]` `register` pages to Supabase Auth directly via `@supabase/ssr` (PRD §7.3 — not through `apps/api`).
...
4. Tenant-branded password-reset/OTP email templates, configured through Supabase Auth's template settings per tenant where supported.
