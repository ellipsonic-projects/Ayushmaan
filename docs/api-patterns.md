...
No raw Storage credentials returned. Uploads/downloads go through `apps/api`-issued signed URLs scoped to `cases/{tenantSlug}/{caseId}/...`, short-lived (5–15 min), type/size-validated server-side.
...
- **Roles:** none yet (public self-service signup) — the caller becomes `TENANT_ADMIN` of the tenant it creates
- **Auth Header:** `Authorization: Bearer <supabase_access_token>` (required — issued by Supabase Auth at signup, before any `users` row exists)
- **Policy:** Self-service tenant provisioning by an admin who set their own password via Supabase `signUp`. This is a deliberate addition alongside §4's `POST /platform/tenants`, which remains the Super-Admin invite-based path for provisioning a tenant on someone else's behalf. 409 if the slug is taken or the calling identity is already registered.
- **Description:** Creates `tenants` + `tenant_settings` + `tenant_billing` + a `TENANT_ADMIN` `users` row for the caller, in one transaction.
...
- **Roles:** `SUPER_ADMIN`
- **Auth Header:** `Authorization: Bearer <supabase_access_token>` (required)
- **Policy:** Super Admin token provisions a tenant on a customer's behalf, inviting the `TENANT_ADMIN` by email (PRD §1.4). This is no longer the only way to create a tenant — see `POST /auth/register-tenant` in §3 for the self-service path where the admin sets their own password.
...
- **Roles:** self
- **Auth Header:** `Authorization: Bearer <supabase_access_token>` (required)
- **Policy:** Returns a signed Storage upload URL scoped to `{tenantSlug}/{consultantId}/...`, valid 5–15 min; server-side file-type/size validation before the URL is issued.
...
- **Auth Header:** `Authorization: Bearer <supabase_access_token>` (required)
- **Policy:** Returns a signed upload URL scoped to `cases/{tenantSlug}/{caseId}/...`; `isClientVisible` is set by the uploader at this step and is immutable after `confirm` (new versions only, never a retroactive flip).
