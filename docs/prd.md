...
- **Storage isolation:** Supabase Storage buckets use a `cases/{tenantSlug}/{caseId}/...` (case documents) / `{tenantSlug}/{consultantId}/...` (consultant verification) path prefix; signed URLs are generated per-request and scoped to that prefix only.
...
| `app/(tenant)/[slug]/(admin)/appointments/page.tsx`      | **New:** Tenant Admin's incoming requests queue — first stop for every new booking; Approve (forwards to the Consultant) / Propose Reschedule / Reject with reason, checking the Consultant's availability before forwarding.                                                                                                                       |
| `app/(tenant)/[slug]/(consultant)/appointments/page.tsx` | Consultant's queue of Tenant-Admin-approved requests: Accept / Reject with reason; **new:** morning-briefing card at top (Phase-2 everyday-life feature #1); **new:** approve/manage a recurring series request as a single action rather than approving each occurrence individually.                                                              |
