...
|---|---|
| `apps/web/middleware.ts` | Subdomain → tenant resolution for **UI routing only**; blocks requests for `SUSPENDED`/unknown tenants with a branded "this practice is unavailable" page. |
...
- **RLS becomes the single source of truth for isolation**, straight off the JWT — the exact mechanism already specified in §1.2 — rather than needing tenant checks duplicated in Express middleware *and* the database. This is the main reason a custom JWT/bcrypt implementation would be **harder** here: it would mean rebuilding password hashing, token issuance/rotation, and OTP delivery yourself, and still wiring tenant scoping into Postgres by hand.
