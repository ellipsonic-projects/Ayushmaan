...
-- `workflows` is handled by its own scope-gated policy in 13-workflows.sql
-- (like workflow_templates) — tenant_id is nullable there for a
-- SUPER_ADMIN-authored COMMUNITY row, which this generic policy's plain
-- tenant_id match can't make visible across tenants.
...
declare
  ...
  tables text[] := array[
    ...
    'payments',
    'contacts',
    'case_consultant_assignments',
    'workflow_runs',
    'form_submissions',
    'shared_templates',
    'tenant_availability_defaults'
  ];
begin
